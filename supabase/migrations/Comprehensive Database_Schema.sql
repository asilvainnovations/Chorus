-- ========================================================================
-- CHORUS: Strategic Foresight Platform
-- Comprehensive Database Schema
-- ========================================================================
-- This schema supports:
-- - Multi-user chat conversations with domain context
-- - 10 interconnected global challenge domains
-- - Domain-aware AI system prompts and frameworks
-- - Message citations and grounded research tracking
-- - Web search results and source attribution
-- - Model preferences and user settings
-- ========================================================================
...
CREATE INDEX idx_users_auth_provider_auth_id ON users(auth_provider, auth_id);
...
-- Domain-specific frameworks (UN SDGs, UNDRR Sendai, Ellen MacArthur, etc.)
CREATE TABLE domain_frameworks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_domain_frameworks_domain_id ON domain_frameworks(domain_id);

-- Domain resources (case studies, research, tools, initiatives)
...
CREATE INDEX idx_domain_resources_domain_id ON domain_resources(domain_id);
CREATE INDEX idx_domain_resources_type ON domain_resources(type);
...
CREATE INDEX idx_domain_connections_domain_id_1 ON domain_connections(domain_id_1);
CREATE INDEX idx_domain_connections_domain_id_2 ON domain_connections(domain_id_2);

-- Suggested queries for each domain
CREATE TABLE domain_suggested_queries (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  domain_id VARCHAR(50) NOT NULL REFERENCES domains(id) ON DELETE CASCADE,
...
CREATE INDEX idx_domain_suggested_queries_domain_id ON domain_suggested_queries(domain_id);
...
CREATE INDEX idx_conversations_active_domain_id ON conversations(active_domain_id);
...
-- Message metadata (streaming status, errors, etc.)
...
CREATE INDEX idx_message_sources_source_url ON message_sources(source_url);

-- Web search results (for tracking and analytics)
...
-- Model preferences (which models user has access to)
...
-- API key management (for users bringing their own keys)
...
CREATE INDEX idx_daily_usage_summary_user_id ON daily_usage_summary(user_id);
...
CREATE INDEX idx_conversation_shares_owner_user_id ON conversation_shares(owner_user_id);
...
CREATE VIEW conversation_with_context AS
SELECT
  c.id,
  c.user_id,
  c.title,
...
CREATE VIEW user_message_statistics AS
SELECT
  u.id AS user_id,
  u.email,
...
CREATE VIEW domain_popularity AS
SELECT
  d.id,
  d.name,
...
CREATE VIEW message_source_effectiveness AS
SELECT
  s.source_domain,
...
CREATE INDEX idx_messages_conversation_domain_created ON messages(
  conversation_id, domain_id, created_at DESC
);

CREATE INDEX idx_conversations_user_domain_active ON conversations(
  user_id, active_domain_id, deleted_at
);

CREATE INDEX idx_usage_analytics_user_date ON usage_analytics(
  user_id, created_at DESC
);

-- Full-text search indexes
CREATE INDEX idx_message_content_fts ON messages USING GIN (
  to_tsvector('english', content)
);

CREATE INDEX idx_conversation_title_fts ON conversations USING GIN (
